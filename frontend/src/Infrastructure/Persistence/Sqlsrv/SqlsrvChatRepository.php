...
class SqlsrvChatRepository implements ChatRepositoryInterface {
    ...
    public function saveMensaje(ChatMensaje $mensaje): ChatMensaje {
        // 1. Clasificación automática si es un mensaje enviado por el usuario
        if (strtolower($mensaje->tipo_remitente) === 'usuario') {
            $rInc = sqlsrv_query($this->conn, "SELECT asunto FROM dbo.Incidencia WHERE id_incidencia = ?", [(int)$mensaje->conversacion_id]);
            if ($rInc && $rowInc = sqlsrv_fetch_array($rInc, SQLSRV_FETCH_ASSOC)) {
                $asunto = $rowInc['asunto'] ?? '';
                // Solo auto-clasificar si la incidencia sigue teniendo el asunto inicial genérico
                if (strpos($asunto, 'Soporte en') === 0) {
                    $classified = \App\Domain\Services\AutoClassifier::classify($mensaje->contenido);
                    
                    if ($classified['has_matches']) {
